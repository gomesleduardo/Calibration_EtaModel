...
for (data.c in 1:4)
{
    ...
    Ft.new[,,t] <- t(data.frame(level = 1,
                            ...
                            AR = c(as.numeric(subset(obs.wind,obs.wind$data.hora ==
                                                     as.character(strptime(paste0(forecast.period[t-1]," ",forecast.hour,":00:01"),
                                                                           format = "%Y-%m-%d %H:%M:%S",
                                                                           tz = "GMT")-(horizon/24)*24*60*60))[,-1]),rep(0,2937)),
                            alt = c(coords.stations$altitude,rep(0,2937)),
  ...
  for (m in 1:M.post)
  {
    ...
    Rt.k[,,1] <- Ht.final[,,m]
    mat.cor.new <- MatCor(phi = PSI.chain$phi[m], DistMatrix = D)
    ...
    for (k in 2:(T.new+1))
    {
      ...
      Qt.k <- symmetricMatrix(t(Ft.new[,parc.prev,k])%*%Rt.k[,,k]%*%Ft.new[,parc.prev,k]+sigma2t.k*mat.cor.new)
