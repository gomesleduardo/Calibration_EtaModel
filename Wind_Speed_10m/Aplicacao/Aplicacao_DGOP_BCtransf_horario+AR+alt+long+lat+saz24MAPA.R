...
for (data.c in 1:12)
{
  ...
  aux.date <- seq(as.Date(head(unique(substr(train.period,1,10)),n=1))-10,
  ...
  Rcpp::sourceCpp("C:/Users/b207056565/Desktop/Calibration_EtaModel/Funcs_DGOP.cpp")
  ...
  Desc <- diag(c(1/sqrt(.99),1/sqrt(.99),1/sqrt(.99),1/sqrt(.99),1/sqrt(.99),1/sqrt(.99),1/sqrt(.99),1/sqrt(.99)))
  ...
  #----------------------------------------------------------------------------------------------
  wind10m.ETA <- data.frame(NULL)
  aux.date <- seq(as.Date(head(unique(substr(forecast.period,1,10)),n=1))-10,
  ...
  for (t in 2:(T.new+1))
  {
    ...
    Ft.new[,,t] <- t(data.frame(level = 1,
                            ensemble.mean = apply(subset(wind10m.ETA, 
                                                         wind10m.ETA$data == forecast.period[t-1])[,-(1:3)],2,mean),
                            AR = c(as.numeric(subset(obs.wind,obs.wind$data.hora ==
                                                       as.character(strptime(paste0(forecast.period[t-1]," ",forecast.hour,":00:01"),
                                                                             format = "%Y-%m-%d %H:%M:%S",
                                                                             tz = "GMT")-(horizon/24)*24*60*60))[,-1]),
                                   ifelse(is.na(suppressWarnings(interpp(x = coords.stations$longitude,
                                                                         y = coords.stations$latitude,
                                                                         z = as.numeric(subset(obs.wind,obs.wind$data.hora ==
                                                                                                 as.character(strptime(paste0(forecast.period[t-1]," ",forecast.hour,":00:01"),
                                                                                                                       format = "%Y-%m-%d %H:%M:%S",
                                                                                                                       tz = "GMT")-(horizon/24)*24*60*60))[,-1]),
                                                                         xo = coords.ETA$longitude,
                                                                         yo = coords.ETA$latitude)$z)),2,
                                          suppressWarnings(interpp(x = coords.stations$longitude,
                                                                   y = coords.stations$latitude,
                                                                   z = as.numeric(subset(obs.wind,obs.wind$data.hora ==
                                                                                           as.character(strptime(paste0(forecast.period[t-1]," ",forecast.hour,":00:01"),
                                                                                                                 format = "%Y-%m-%d %H:%M:%S",
                                                                                                                 tz = "GMT")-(horizon/24)*24*60*60))[,-1]),
                                                                   xo = coords.ETA$longitude,
                                                                   yo = coords.ETA$latitude)$z))),
                            alt = c(coords.stations$altitude,ifelse(is.na(suppressWarnings(interpp(x = coords.stations$longitude,
                                                                                                   y = coords.stations$latitude,
                                                                                                   z = coords.stations$altitude,
                                                                                                   xo = coords.ETA$longitude,
                                                                                                   yo = coords.ETA$latitude)$z)),700,
                                                                    suppressWarnings(interpp(x = coords.stations$longitude,
                                                                                             y = coords.stations$latitude,
                                                                                             z = coords.stations$altitude,
                                                                                             xo = coords.ETA$longitude,
                                                                                             yo = coords.ETA$latitude)$z))),
                            lat = c(coords.stations$latitude,coords.ETA$latitude),
                            long = c(coords.stations$longitude,coords.ETA$longitude),
                            saz1 = 1,
                            saz2 = 0))
    
    Ft.numforecast[,,t] <- t(data.frame(level = 1,
                                        ensemble.mean = apply(tail(subset(wind10m.ETA, 
                                                                     wind10m.ETA$data == forecast.period[t-1]),n=1)[,-(1:3)],2,mean),
                                        AR = c(as.numeric(subset(obs.wind,obs.wind$data.hora ==
                                                                   as.character(strptime(paste0(forecast.period[t-1]," ",forecast.hour,":00:01"),
                                                                                         format = "%Y-%m-%d %H:%M:%S",
                                                                                         tz = "GMT")-(horizon/24)*24*60*60))[,-1]),
  ...
  ht.final <- ht.final[-(1:burnin),]
  Ht.final <- Ht.final[,,-(1:burnin)]
  sigma2t.chain.final <- sigma2t.chain.final[,-(1:burnin)]
  
  #------ Atualizando desconto da variancia ------#
  nt.k <- 1
  for (k in 2:(T+1+T.new)){nt.k[k] <- delta.sig2*nt.k[k-1] + 1}
  nt.k <- nt.k[(T+1):(T+1+T.new)]
  
  ##############################################
  ###### PREVENDO APENAS PARA AS ESTACOES ######
  ##############################################
  ...
  for (m in 1:M.post)
  {
    ...
    Rt.k[,,1] <- Ht.final[,,m]
    mat.cor.new <- MatCor(phi = PSI.chain$phi[m], DistMatrix = D.new)
    ...
    for (k in 2:(T.new+1))
    {
      ...
      Qt.k <- symmetricMatrix(t(Ft.new[,,k])%*%Rt.k[,,k]%*%Ft.new[,,k]+sigma2t.k*mat.cor.new)
  ...
  # for (l in 1:59)
  # {
  #   plot(0,type="n",ylim=c(0,16),xlim=c(1,(T.new+1)),
  #        ylab=coords.stations$codigo[l],xlab="horizonte", axes= F)
  #   axis(1,1:25,c("0",paste0("+",1:24)))
  #   axis(3,1:25,paste0(c(12:23,0:12),"h"))
  #   abline(h=0)
  ...
  #   abline(v=1:25,lty=2,col="grey80")
  #   axis(2,0:16,0:16)
  ...
  #   lines(2:(T.new+1),Ft.numforecast[2,l,2:(T.new+1)],lty=2,lwd=2,col=4)
  # }
  # 
  # plot.new()
  ...
  rm(wind10m.ETA)
  rm(D.new)
  save.image(paste0("C:/Users/b207056565/Desktop/Calibration_EtaModel/Wind_Speed_10m/Aplicacao/Resultados/Horarios/mapa",
                    datas[data.c],"DGOP.Rda"))
