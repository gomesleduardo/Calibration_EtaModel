...
Gt <- as.matrix(bdiag(diag(1,q+1),
                      ...
                      matrix(c(cos(2*pi*2/24),-sin(2*pi*2/24),
                               sin(2*pi*2/24),cos(2*pi*2/24)),2)))
...
#------ Thetat ------#
set.seed(1)
Thetat <- data.frame(t(c(-.3,.7,0,0,0,0)))
...
for (m in 1:r){
  ...
  abline(v=seq(24,T,24),lty=2,col="gray70")
  abline(v=seq(12,T,12),lty=3,col="gray90")}
...
Desc <- diag(c(1/sqrt(.9),1/sqrt(.85),rep(1/sqrt(.95),4)))
